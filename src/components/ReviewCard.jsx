...
const ReviewCard = ({
  ...
  const rating = author_details?.rating ?? 0;
            ...
            <div className="mb-5 flex w-full justify-between">
              {rating !== null && isUser ? (
                <div>
                  ...
                  />
                  {isProfileList && (
                    <p className="text-slate-400 md:text-sm text-xs mt-2">
                      Date written: {updateDateFormat(created_at)}
                    </p>
                  )}
                </div>
              ) : (
                <RatingBox
                  rating={rating}
                  styleOfSpan="w-[32px] text-sm rounded-sm bg-orange-amber font-semibold text-gray-800"
                  initialStars={5}
                />
              )}
